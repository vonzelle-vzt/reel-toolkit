...
export const SHOOTING_TIPS: ShootingTip[] = [
...
export function getRailTips(limit = 4): ShootingTip[] {
...
export function getTipsByCategory(): Record<TipCategory, ShootingTip[]> {
  const result = {} as Record<TipCategory, ShootingTip[]>;
  for (const cat of TIP_CATEGORY_ORDER) {
    result[cat] = SHOOTING_TIPS.filter((t) => t.category === cat).sort(
