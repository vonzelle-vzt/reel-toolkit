 ...
 */
const MIDDLE_TAIL_BY_REEL_TYPE: Record<string, number> = {
  SOCIAL_VERTICAL: 1.5,
...
export function getClipPlayableWindow(
  clip: ClipForWindow,
  reelType: string | null | undefined,
  ...
  const hasEventEnd =
    typeof clip.eventEndTime === 'number' &&
    clip.eventEndTime > startTime &&
    clip.eventEndTime <= endTime;

  if (isLastClip && hasEventEnd) {
    endTime = Math.min(endTime, (clip.eventEndTime as number) + LAST_CLIP_TAIL_SECONDS);
  } else if (!isLastClip && hasEventEnd) {
    const tail = clip.endTime - (clip.eventEndTime as number);
    if (tail > MIDDLE_CAP_TRIGGER_SECONDS) {
      const budget = reelType
        ? (MIDDLE_TAIL_BY_REEL_TYPE[reelType] ?? DEFAULT_MIDDLE_TAIL)
        : DEFAULT_MIDDLE_TAIL;
      endTime = Math.min(endTime, (clip.eventEndTime as number) + budget);
    }
  }
