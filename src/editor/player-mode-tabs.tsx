...
const DEFAULT_LABELS: Record<PlayerMode, string> = {
  clips: 'Clips',
  produced: 'Highlight',
  movie: 'Movie',
};

const DEFAULT_MODES: ReadonlyArray<PlayerMode> = ['clips', 'produced', 'movie'];

export function PlayerModeTabs({
  mode,
  onChange,
  modes = DEFAULT_MODES,
  labelOverrides,
  className,
}: PlayerModeTabsProps): React.ReactElement {
  return (
    <div
      className={cn(
        'flex gap-1 rounded-lg border border-zinc-700 bg-zinc-900 p-1 w-fit',
        className,
      )}
      role="tablist"
      aria-label="Reel playback mode"
    >
      {modes.map((m) => (
        <button
          key={m}
        ...
        >
          {labelOverrides?.[m] ?? DEFAULT_LABELS[m]}
        </button>
      ))}
    </div>
  );
