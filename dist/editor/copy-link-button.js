...
export function CopyLinkButton({ shareToken, pathPrefix = '/reel/', onSuccess, onError, className, }) {
    ...
    return (_jsxs("button", { type: "button", onClick: handleClick, "aria-label": "Copy share link", title: "Copy share link", className: cn('absolute bottom-2 right-2 z-20 flex h-7 items-center gap-1 rounded-full bg-black/70 px-2.5 text-[10px] font-medium text-white shadow-lg backdrop-blur-sm transition-all hover:bg-amber-500 sm:h-6 sm:opacity-0 sm:group-hover:opacity-100', className), children: [copied ? _jsx(Check, { className: "h-3 w-3" }) : _jsx(Link2, { className: "h-3 w-3" }), copied ? 'Copied' : 'Share link'] }));
}
